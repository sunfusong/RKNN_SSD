...
public class ImageBufferQueue {
    private ImageBuffer[] mQueueBuffer;//存放生成的TextureId
    ...
    public ImageBufferQueue(int bufferSize, int width, int height) {
        ...
        mQueueBufferSize = bufferSize;//3
        mQueueBuffer = new ImageBuffer[mQueueBufferSize];

        for (int i=0; i<mQueueBufferSize; ++i) {
            mQueueBuffer[i] = new ImageBuffer(width, height);//创建纹理InferenceWrapper.create_direct_texture
        }
    }
    ...
    public synchronized ImageBuffer getReadyBuffer() {
        ...
        int index = mCurrentUsingBufferIndex;//-1

        for (int i=0; i<mQueueBufferSize; ++i) {//mQueueBufferSize==3
            ++index;
    ...
    public class ImageBuffer {
        ...
        public ImageBuffer(int width, int height) {
            ...
            mHeight = height;
            mTextureId = -1;
            prepareFrameBuffer(width, height);//创建纹理
        }
        ...
        public void finalize() {
            if (mTextureId >= 0) {
                ...
                glDeleteFramebuffers(1, values, 0);//删除命名的帧缓冲区对象
                InferenceWrapper.delete_direct_texture(mTextureId);//删除纹理
            }
        }
        ...
        private void prepareFrameBuffer(int width, int height) {
            ...
            mTextureId = InferenceWrapper.create_direct_texture(width, height, GL_RGB);//创建纹理

            glBindTexture(GL_TEXTURE_2D, mTextureId);//绑定纹理
            ...
            glGenFramebuffers(1, values, 0);//gl自动生成Framebuffers
            mFramebuffer = values[0];    // expected > 0
            glBindFramebuffer(GL_FRAMEBUFFER, mFramebuffer);

            glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
                    GL_TEXTURE_2D, mTextureId, 0);//将纹理绑定到帧缓冲区中
