...
public class InferenceWrapper {
    ...
    * */
    public InferenceWrapper(int inputSize, int channel, int numResult, int numClasses, String modelPath) {
        ...
        mOutputs.mLocations = new float[numResult * 4];//创建输出的Location容器
        mOutputs.mClasses = new float[numResult * numClasses];//创建输出的分类容器

        init(inputSize, channel, numResult, numClasses, modelPath);//对参数进行初始化
    }
